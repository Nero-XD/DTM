...
class LNPBlock(nn.Module):
    ...
    def forward(self, center, feat):
        ...
        lc_x_w = self.lga(center, feat)  # B 2C G K  %%K_Norm
...
class DTMEncoder(nn.Module):
    def __init__(self, k_group_size=8, embed_dim=768, depth=4, attention_ratio=0.3, drop_path_rate=0., num_group=128,
                 num_heads=6,
                 bimamba_type="v2", ):
        super().__init__()
        self.num_group = num_group
        ...
        self.attention_depth = int(attention_ratio * depth)
        self.blocks = nn.ModuleList([
            AttentionBlock(
                ...
                k_group_size=self.k_group_size,
                num_group=self.num_group,
                drop_path=drop_path_rate[i + self.attention_depth] if isinstance(drop_path_rate,
                                                                                 list) else drop_path_rate,
            )
            if i < self.attention_depth else
            DTMBlock(
                dim=embed_dim,
                k_group_size=self.k_group_size,
                drop_path=drop_path_rate[i + self.attention_depth] if isinstance(drop_path_rate,
            ...
            for i in range(depth - self.attention_depth)])
...
class DTM(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.attention_ratio = config.attention_ratio
