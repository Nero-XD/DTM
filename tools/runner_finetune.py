...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    #model param
    param_cnt = calculate_total_param(base_model)
    ...
    print_log('[PARAM: %.3f M]' % (param_cnt / 1000000), logger=logger)
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        ...
        category_correct_count = {i: 0 for i in range(40)}
        category_total_count = {i: 0 for i in range(40)}
        for label, pred in zip(test_label, test_pred):
            category_total_count[label.item()] += 1
            if label == pred:
                category_correct_count[label.item()] += 1

        for i, cat_name in enumerate(test_dataloader.dataset.cat):
            correct = category_correct_count[i]
            total = category_total_count[i]
            accuracy = correct / total if total > 0 else 0
            print_log(f"种类: {cat_name}, 预测正确数: {correct}, 总数: {total}, 正确率: {accuracy:.2%}", logger=logger)
